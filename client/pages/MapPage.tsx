...
const MapPage: React.FC = () => {
  ...
  const [selectedPlace, setSelectedPlace] = React.useState<{lat:number,lng:number,display_name?:string}|null>(null);
  const [reportOpen, setReportOpen] = React.useState(false);
  const [pickLocation, setPickLocation] = React.useState<{lat:number,lng:number}|null>(null);
  ...
  return (
    <div className="min-h-[80vh] relative">
            ...
            <aside className="w-80 p-4 rounded border bg-card">
              ...
              <div className="mb-3">
                ...
                <div className="mt-2 max-h-48 overflow-auto">
                  {suggestions.map((s:any)=> (
                    <div key={s.place_id} className="p-2 hover:bg-muted-foreground/5 rounded flex justify-between items-center">
                      <div>
                        <div className="text-sm">{s.display_name}</div>
                        <div className="text-xs text-muted-foreground">{s.type} �� {s.class}</div>
                      </div>
                      <div className="flex flex-col gap-1">
                        <button className="text-xs px-2 py-1 rounded bg-ocean-600 text-white" onClick={()=>{
                          const lat = parseFloat(s.lat); const lng = parseFloat(s.lon);
                          setSelectedPlace({lat,lng,display_name:s.display_name});
                          setPickLocation({lat,lng});
                        }}>
                          Use
                        </button>
              ...
              <div className="mb-3">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={selectLocationMode} onChange={(e)=>setSelectLocationMode(e.target.checked)} />
                  <span className="text-sm">Select location on map</span>
                </label>
                <div className="mt-2 text-xs text-muted-foreground">When enabled, click anywhere on the map to choose report location.</div>
              </div>

              <div className="flex gap-2 mt-4">
                <Button onClick={()=>setReportOpen(true)}>Report at Selected</Button>
                <Button variant="ghost" onClick={()=>{ setFiltersOpen(false); setSearchQuery(''); setSuggestions([]); }}>Close</Button>
              </div>
            </aside>
          )}
      ...
      <div className="fixed right-6 bottom-6 md:bottom-8 z-50">
        <Button className="rounded-full" size="lg" onClick={()=>{ setFiltersOpen(true); setReportOpen(true); }}>Report</Button>
      </div>
      ...
      <Dialog open={reportOpen} onOpenChange={setReportOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <ReportForm pickLocation={pickLocation} onSubmitted={(r)=>{ setReportOpen(false); setPickLocation(null); setSelectedPlace(null); setFiltersOpen(false); setActiveHazardsCount((c)=>c+1); }} />
        </DialogContent>
      </Dialog>
    </div>
  );
