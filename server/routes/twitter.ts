  ...
  try {
    ...
    if (process.env.TWITTER_ENABLED !== "true") {
      ...
      return res.json({
        data: items,
        includes: {},
        meta: { fallback: true, reason: "disabled" },
    ...
    if (twitterRateLimitedUntil && now < twitterRateLimitedUntil) {
      console.warn(
        "Twitter API rate-limited, returning fallback data until",
        twitterRateLimitedUntil,
      );
      const items = makeFallbackSocial(query);
      return res.json({
        data: items,
    ...
    const result: TwitterSearchResponse =
      await twitterService.searchTweets(params);
    res.json(result);
  } catch (error: any) {
    ...
    if (
      ...
      (error?.type === "response" && error?.code === 429)
    ) {
      const reset = error?.headers?.["x-rate-limit-reset"]
        ? parseInt(error.headers["x-rate-limit-reset"], 10)
        : null;
...
export const searchOceanEmergencyTweets: RequestHandler = async (req, res) => {
  try {
    ...
    if (!twitterService) return;
    const result: TwitterSearchResponse =
      await twitterService.searchOceanEmergencyTweets(
        location as string,
...
export const getOceanEmergencySocialFeed: RequestHandler = async (req, res) => {
  try {
    ...
    if (process.env.TWITTER_ENABLED !== "true") {
      console.warn(
        "TWITTER_ENABLED not true — returning local fallback social feed",
      );
      const items = makeFallbackSocial(location as string | undefined).map(
        (it, i) => ({
          id: it.id,
    ...
    if (
      ...
      const reset = error?.headers?.["x-rate-limit-reset"]
        ...
        : null;
      if (reset) twitterRateLimitedUntil = reset;
      else twitterRateLimitedUntil = Math.floor(Date.now() / 1000) + 60;
      const items = makeFallbackSocial(
    ...
    res
      .status(500)
      .json({ error: "Failed to get ocean emergency social feed" });
  }
};
...
export const postTweet: RequestHandler = async (req, res) => {
  try {
    ...
    if (text.length > 280) {
      return res
        .status(400)
        .json({ error: "Tweet text exceeds 280 characters" });
    }
